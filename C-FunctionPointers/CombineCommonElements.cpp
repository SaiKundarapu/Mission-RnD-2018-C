...
floatObjectNode->equals = <Any Address of function which checks the equality of 2 void * who will point to 2 float values>
...
struct ObjectNode * createIntObjectNode(int *number, int(*equals)(void *, void *))
{
	...
	struct ObjectNode * intObjectNode = (struct ObjectNode *)malloc(sizeof(struct ObjectNode));
	intObjectNode->value = (void *)&number;
	intObjectNode->equals = equals;
	return intObjectNode;
}
...
struct ObjectNode ** combine_common_elements(struct ObjectNode ** objectNodes1, struct ObjectNode ** objectNodes2, int len1, int len2, int *commonElementsLength)
{
	...
	for (i = 0; i < len1; i++)
	{
		for ( x = 0; x < k; x++)
		{
			if (objectNodes1[i]->equals(objectNodes1[i]->value,result[x]->value) == 1)
			{
				break;
			}
		}
