...
Ex : If Hack'hacklist List is "CADBZ" , He wants to go first to C island ,next A island and B and so on ,and finally
reach Z island .
...
char * find_common_route(char * hacklist, char *codelist, int *cost)
{
	*cost = 0;
	...
	int hacklen = slen(hacklist);
	int codelen = slen(codelist);

	int maxlen = 0;
	int index = 0;
	int len = (hacklen< codelen) ? codelen : hacklen;
	char* out = (char*)malloc(len + 1);

	int ** matrix = (int**)calloc(sizeof(int), hacklen + 1);
	for (int i = 0; i < hacklen + 1; i++)
		matrix[i] = (int*)calloc(sizeof(int), codelen + 1);
