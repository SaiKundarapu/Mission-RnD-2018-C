...
void findmin(struct student *leaderstudent,int count,int *min)
{
	count++;
	if (leaderstudent->first == NULL)
	{
		...
		findmin(leaderstudent->first, count, min);
	if (leaderstudent->second == NULL)
	{
		if ( count < *min)
			*min = count;
	}
	else
		findmin(leaderstudent->second, count, min);
	if (leaderstudent->third == NULL)
	{
		if (count < *min)
			*min = count;
	}
	else
		findmin(leaderstudent->third, count, min);

	return;
}
int findmax(struct student *leaderstudent)
{
	if (leaderstudent == NULL)
		return 0;
	else
	{
		int f = findmax(leaderstudent->first);
		int s = findmax(leaderstudent->second);
		int t = findmax(leaderstudent->third);
		return max(f, s, t)+1;
	}
...
void findBallDistance(struct student *leaderStudent, int *minBallDistance, int *maxBallDistance)
{
	...
	{
		int min = 9999;
		*maxBallDistance = findmax(leaderStudent);
		findmin(leaderStudent, 0, &min);
		*minBallDistance = min;
	}
	return;
}